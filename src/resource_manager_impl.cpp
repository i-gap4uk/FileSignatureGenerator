...
ResourceManagerImpl::ResourceManagerImpl()
    : last_data_block_id_(0), last_hash_id_(0), last_written_hash_id_(0)
{
}

ResourceManagerImpl::~ResourceManagerImpl() {}

void ResourceManagerImpl::InsertDataBlock(
    const types::file_manager::DataContainer & data_block)
{
...
void ResourceManagerImpl::InsertHash(
    const types::hash_generator::HashContainer & hash)
{
...
bool ResourceManagerImpl::GetHash(
...
bool ResourceManagerImpl::GetDataBlock(
    types::file_manager::DataContainer & data_block)
{
    std::lock_guard<std::mutex> lock(data_queue_mutex_);
