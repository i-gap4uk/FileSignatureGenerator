...
FileManagerImpl::FileManagerImpl(const std::string & output_file,
                                 const std::string & input_file)
    : file_size_(0), already_read_size_(0), data_block_id_(0),
...
types::Result
FileManagerImpl::ReadDataBlock(types::file_manager::DataContainer & data_block)
{
    using namespace types::file_manager;
    ...
    try
    {
        ...
        if (remined_size < block_size)
        {
            ...
            input_stream_.read(&(data_block.data[0]),
                               static_cast<long>(remined_size));
            std::cout << "Block count: " << data_block_id_ << std::endl;
            already_read_size_ +=
                static_cast<std::size_t>(input_stream_.gcount());
            return types::Result::END_OF_FILE;
        }

        input_stream_.read(&(data_block.data[0]),
                           static_cast<long>(data_block.data.size()));

        already_read_size_ += static_cast<std::size_t>(input_stream_.gcount());
        return types::Result::DATA_IS_READ;
    }
    catch (std::exception e)
    {
...
bool FileManagerImpl::OpenOutputFile()
{
    ...
    output_steam_.open(output_file_path_,
